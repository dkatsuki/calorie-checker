...
module ImageAttachment
  ...
  module ClassMethods
    ...
    def save_binary_to_system_tmp(binary, extention)
      extention.gsub!(/\./, '')
      tmp_file_path = "/tmp/#{SecureRandom.uuid}.#{extention}"
  ...
  def set_image_file_as_action_dispatch_to_image_key(file_path)
    set_image_file_as_action_dispatch_to(:main_image_key, file_path)
  end
  ...
  def image_source
    if self.main_image_key.present?
      "https://#{self.class.files_domain}/#{self.main_image_key}"
    else
      ''
    end
  end
  ...
  private
    def make_s3_key(attribute_name)
      ...
      result = result + '/' + attribute_name if attribute_name != 'main_image_key'
