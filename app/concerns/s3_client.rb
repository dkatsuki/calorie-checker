class S3Client
	...
	def initialize(config = nil, bucket_name = nil)
		if config.blank?
		...
		@default_bucket = bucket_name || config[:s3][:default_bucket]
	...
	def get_objects(bucket: nil, prefix: nil, next_objects_token: nil)
		...
		bucket = @default_bucket if bucket.blank?
	...
	def put_object(bucket: nil, file_name: nil, body: nil, acl: 'public-read')
		bucket = @default_bucket if bucket.blank?
		return false if bucket.blank? || file_name.blank?
		is_old_file_exist = file_exist?(file_name: file_name, bucket: bucket)
		@client.put_object(bucket: bucket, key: file_name, body: body, acl: acl)
	...
	def search_file_names(bucket: nil, prefix: nil)
		...
		object_list = @client.list_objects_v2(bucket: @default_bucket, prefix: prefix)
		object_list.contents.each do |object|
			file_names << object.key
		end

		while object_list.is_truncated
			object_list = @client.list_objects_v2(bucket: @default_bucket, prefix: prefix, continuation_token: object_list.next_continuation_token)
			object_list.contents.each do |object|
				file_names << object.key
			end
		end
